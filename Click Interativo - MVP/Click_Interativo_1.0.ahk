...
SendMode "Input"
CoordMode "Mouse", "Client"  ; Click/MouseGetPos ficam relativos à área cliente da janela ativa. (CoordMode)
...
; Lista de pontos
lv := g.Add("ListView", "xm w860 h280 Grid", ["ID","Janela/Aplicação","x","y","Modo","ClassNN","Intervalo(ms)","Ativar","Ativo"])
...
AddPointWizard(*) {
    ...
    ; Handler robusto: função separada (OnEvent requer uma função/callback)
    btCap.OnEvent("Click", CapturePoint.Bind(winHwnds, ddl, edX, edY))  ; passa args via Bind()  (Gui.OnEvent)
...
CapturePoint(winHwnds, ddl, edX, edY, *) {
    ...
    ActivateWindow("ahk_id " winHwnds[idx])     ; ativa e aguarda ficar ativo (WinActivate/WinWaitActive)
    ToolTip "Clique e solte no ponto de interação..."
    KeyWait "LButton", "D"                       ; espera pressionar (KeyWait)
    KeyWait "LButton", "U"                       ; espera soltar
    ToolTip                                      ; apaga tooltip
    MouseGetPos &mx, &my                         ; lê posição atual (MouseGetPos)
...
StartCycle(*) {
    ...
    ; Define fim opcional (DateTime fornece timestamp compatível YYYYMMDDHH24MISS)
    if (cbUseEnd.Value=1) {
        gEndAt := dtEnd.Value  ; comparar com A_Now
    } else {
        gEndAt := ""
    }
    ; inicializa nextDue de cada ponto
    now := A_TickCount
    for p in gPoints {
        if (p["active"])
            p["nextDue"] := now
    ...
    SetTimer SchedulerTick, gTickMs         ; agenda chamadas periódicas não-bloqueantes (SetTimer)
}
...
SchedulerTick(*) {
    ...
    ; Encerramento por data/hora
    if (gEndAt != "" && A_Now >= gEndAt) {      ; YYYYMMDDHH24MISS compara por ordem cronológica
        StopCycle()
        return
    }
...
; Realiza um clique segundo o modo do ponto
DoOneClick(p) {
    try {
        if (p["activateBefore"]) {
            if !ActivateWindow(BuildWinTitle(p))
                return false
        }
        if (p["useCtrl"]) {
            SetControlDelay -1
            if (p["classNN"]!="")
                ControlClick p["classNN"], BuildWinTitle(p), , "Left", 1, "NA"
            else
                ControlClick "x" p["x"] " y" p["y"], BuildWinTitle(p), , "Left", 1, "NA"
        } else {
            Click p["x"], p["y"]
        }
        return true
